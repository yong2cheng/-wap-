...
<article>
    <div class="search">
        ...
        <el-button type="primary" style="float: right;margin-top: 4px"><router-link :to="{path:'/article/add'}">添加</router-link></el-button>
    </div>
    <div style="height:calc(100vh - 218px);">
        <el-table ref="multipleTable" :data="blogList" tooltip-effect="dark" stripe border height="100%">
            <el-table-column type="index" width="55" align="center" header-align="center" :index="increment" label="序号"></el-table-column>
    
            <el-table-column show-overflow-tooltip v-if="!item.hidden && !item.filters && !item.filtersType" v-for="(item, index) in headerOptions" :key="index" :label="item.label" :prop="item.prop" :header-align="item.headerAlign" :align="item.align" :sortable="item.sort"  :min-width="item.minWidth || 150">
            ...
            <el-table-column  v-else-if="!item.hidden && item.filters" :key="index" :label="item.label" :prop="item.prop" :header-align="item.headerAlign" :align="item.align" :sortable="item.sort" :filters="item.filters" :filter-method="filterTag"  :min-width="item.minWidth || 200">
                <template slot-scope="scope">
                    <el-tag
                        class="tag"
                        type="primary"
            ...
            <el-table-column  v-else-if="!item.hidden && item.filtersType" :key="index" :label="item.label" :prop="item.prop" :header-align="item.headerAlign" :align="item.align" :sortable="item.sort" :filters="item.filtersType" :filter-method="filterHandler"  :min-width="item.minWidth || 200">
                <template slot-scope="scope">
                    <div>{{scope.row.source === 1?'原创':scope.row.source === 2?'转载':'翻译'}}</div>
                </template>
            </el-table-column>
...
<script>
    ...
    import { blogFilters,sourcesFilters } from 'store/modules/classify'
    export default {
        components: {
            EditComponent
        },
        ...
        methods: {
            ...
            filterHandler(value, row) {
                return sourcesFilters[row.source-1].text == value
            }
        },
        computed: {
            ...mapGetters([
