...
<article>
    ...
    <div style="height:calc(100vh - 220px);">
        <el-table ref="multipleTable" :data="taskRewardlist" tooltip-effect="dark" stripe border height="100%">
            ...
            <el-table-column show-overflow-tooltip v-if="!item.hidden" v-for="(item, index) in headerOptions" :key="index" :label="item.label" :prop="item.prop" :header-align="item.headerAlign" :align="item.align" :sortable="item.sort"  :min-width="item.minWidth || 150">
                <template slot-scope="scope">
                    <div v-if="scope.column.property == 'taskPic'"><img :src="'http://vip.qicheshijia.com.cn/api/'+scope.row[scope.column.property]" style="width:80px;height:40px" @click="clickImg($event)"/></div>
    ...
    export default {
        ...
        methods: {
            ...
            async getTaskReward () {
                this.loading = true;
                try {
                    await this.$store.dispatch('getTaskReward')
                    this.loading = false;
                }catch(e) {
                    this.loading = false;
                }
