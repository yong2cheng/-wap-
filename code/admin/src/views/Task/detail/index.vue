    ...
    <el-dialog title="任务详情" :visible.sync="dialogTableVisible" class="edit-wrapper" @close="close" width="50%" center>
        <el-form ref="form" label-width="100px" class="form">
            ...
            <el-form-item label="任务图片" prop="isVisible" class="left-item">
                <img :src="'http://vip.yuziyule.cn/api/'+item" alt="" style="width:100px;height:50px;margin-right:5px" @click="clickImg($event)" v-for="(item, index) in info.taskPicList" :key="index" v-if="info.taskPicList">
            </el-form-item>
            <el-form-item label="用户上传图片" prop="isVisible" class="left-item" v-if="info.linkType != 1">
                <img :src="'http://vip.yuziyule.cn/api/'+item" alt="" style="width:100px;height:50px;margin-right:5px" @click="clickImg($event)" v-for="(item, index) in info.applyPicList" :key="index" v-if="info.applyPicList">
            </el-form-item>
